...
func (l *linkedList) prepend(n *node) {
	...
	l.headnode = n
	l.headnode.nextnodepointer = temp
	l.length++
}
...
func (l linkedList) detectLoop() bool {
	...
	for l.length != 0 {
		...
		if fast != nil && fast.nextnodepointer != nil {
...
func (l linkedList) getFirstNodeOfLoop(n *node) *node {
	if l.detectLoop() == true {
		slow := l.headnode
		fast := l.headnode
		for l.length != 0 {
			slow = slow.nextnodepointer
			fast = fast.nextnodepointer
			if slow == fast {
				return slow
			}
			l.length--
		...
		fmt.Println("no loop present in the linked list")
		return nil
	}
	return n
}
...
func main() {
	...
	fmt.Println(myList.getFirstNodeOfLoop(n1))
}
